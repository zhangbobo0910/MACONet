...
class MaskCNN_1(nn.Module):
    # This class is kept in its original, unmodified state for baseline comparisons.
    def __init__(self, input_channels, output_channels, kernel_size=3, depth=3, theta=1):
        ...
        self.c1 = MaskConv2d(input_channels, input_channels, kernel_size=kernel_size, padding='same', flag=1,
                             theta=self.theta)
        self.f1 = MaskConv2d(input_channels, input_channels, kernel_size=1, padding='same', flag=3)
        layers1.extend([self.c1, LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU(), self.c1,
                        LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU(), self.f1,
                        LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU()])
        self.c2 = MaskConv2d(input_channels, input_channels, kernel_size=kernel_size, padding='same', flag=1,
                             theta=self.theta)
        self.f2 = MaskConv2d(input_channels, input_channels, kernel_size=1, padding='same', flag=3)
        layers2.extend([self.c2, LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU(), self.c2,
                        LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU(), self.f2,
                        LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU()])
        layers4.extend([MaskConv2d(input_channels, input_channels, kernel_size=3, padding=1, flag=4, stride=2),
                        LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU(),
                        MaskConv2d(input_channels, input_channels, kernel_size=3, padding=1, flag=4, stride=2),
                        LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU()])
        self.cnns1, self.cnns2, self.cnns3, self.cnns4 = nn.ModuleList(layers1), nn.ModuleList(layers2), nn.ModuleList(
            layers3), nn.ModuleList(layers4)
    ...
    def forward(self, x, mask, train):
        _x1, _x2 = x, x
        x_1, x_2, x_3, x_4 = x.clone(), x.clone(), x.clone(), x.clone()
        _x1, _x2, _x3 = x_1, x_2, x_3
        t_hor, t_ver, t_real, t_se = [], [], [], [x_4]
        cnn_type = [0, 3]
        if 0 in cnn_type:
            direction_flag = True;
            direction = 1
            for layer1 in self.cnns1:
                if isinstance(layer1, LayerNorm):
                    ...
                    if isinstance(layer1.conv2d_3, nn.Conv2d):
                        direction_flag = False; x_1 = x_1 + _x1
                    else:
                        layer1.conv2d_3.diff_conv2d_weight = layer1.conv2d_3.diff_conv2d_weight * direction; direction = direction * -1; direction_flag = True
                else:
                    x_1 = layer1(x_1)
                    if direction_flag: x_1 = x_1 + _x1
                    t_ver.append(x_1)
        if 1 in cnn_type:
            direction_flag = True;
            direction = 1
            for layer in self.cnns2:
                if isinstance(layer, LayerNorm):
                    ...
                    if isinstance(layer.conv2d_3, nn.Conv2d):
                        direction_flag = False; x_2 = x_2 + _x2
                    else:
                        layer.conv2d_3.diff_conv2d_weight = layer.conv2d_3.diff_conv2d_weight * direction; direction = direction * -1; direction_flag = True
                else:
                    x_2 = layer(x_2)
                    if direction_flag: x_2 = x_2 + _x2
                    t_hor.append(x_2)
        if 2 in cnn_type:
            direction_flag = True;
            direction = 1
            for layer in self.cnns3:
                if isinstance(layer, LayerNorm):
                    ...
                    if isinstance(layer.conv2d_3, nn.Conv2d):
                        direction_flag = False; x_3 = x_3 + _x3
                    else:
                        layer.conv2d_3.diff_conv2d_weight = layer.conv2d_3.diff_conv2d_weight * direction; direction = direction * -1; direction_flag = True
                else:
                    x_3 = layer(x_3)
                    if direction_flag: x_3 = x_3 + _x3
                    t_real.append(x_3)
        if 3 in cnn_type:
            for layer in self.cnns4:
                if isinstance(layer, LayerNorm):
                    ...
                    x_4 = layer(x_4); t_se.append(x_4)
        for i in [1]:
            i_up = F.upsample(t_se[i], size=(t_se[i - 1].shape[2], t_se[i - 1].shape[3]), mode='nearest')
            if i_up.shape != t_se[i - 1].shape:
                i_up = F.interpolate(i_up, size=t_se[i - 1].shape[2:], mode='bilinear', align_corners=False)
            t_se[0] = self.t4_trans(i_up) + t_se[0]
...
class MaskCNN_2(nn.Module):
    # This is the modified version with Dilated Convolutions in BFM.
    def __init__(self, input_channels, output_channels, kernel_size=3, depth=3, theta=1):
        ...
        # SDM branches (cnns1, cnns2) definitions remain unchanged
        self.c1 = MaskConv2d(input_channels, input_channels, kernel_size=kernel_size, padding='same', flag=1,
                             theta=self.theta)
        self.f1 = MaskConv2d(input_channels, input_channels, kernel_size=1, padding='same', flag=3)
        layers1.extend([self.c1, LayerNorm((1, input_channels, 1, 1), dim_index=1), nn.GELU(), self.c1,
        ...
        # ！！！ CORE MODIFICATION: Redefine BFM (cnns4) with Dilated Convolutions ！！！
        # We no longer downsample with stride, but use dilation to expand the receptive field at full resolution.
        layers4.extend([
            # Block 1, using dilation=1 (standard convolution)
            MaskConv2d(input_channels, input_channels, kernel_size=3, padding=1, flag=4, dilation=1),
            LayerNorm((1, input_channels, 1, 1), dim_index=1),
            nn.GELU(),
            # Block 2, using dilation=2
            MaskConv2d(input_channels, input_channels, kernel_size=3, padding=2, flag=4, dilation=2),
            LayerNorm((1, input_channels, 1, 1), dim_index=1),
            nn.GELU(),
        ])
    ...
    def forward(self, x, mask, train):
            ...
            for layer1 in self.cnns1:
                if isinstance(layer1, LayerNorm):
                    ...
                    if isinstance(layer1.conv2d_3, nn.Conv2d):
                        ...
                        layer1.conv2d_3.diff_conv2d_weight = layer1.conv2d_3.diff_conv2d_weight * direction; direction *= -1; direction_flag = True
                else:
                    x_1 = layer1(x_1)
                    if direction_flag: x_1 = x_1 + _x1
                    t_ver.append(x_1)
            ...
            for layer in self.cnns2:
                if isinstance(layer, LayerNorm):
                    ...
                    if isinstance(layer.conv2d_3, nn.Conv2d):
                        ...
                        layer.conv2d_3.diff_conv2d_weight = layer.conv2d_3.diff_conv2d_weight * direction; direction *= -1; direction_flag = True
                else:
                    x_2 = layer(x_2)
                    if direction_flag: x_2 = x_2 + _x2
                    t_hor.append(x_2)
        ...
        atts_dc1 = torch.cat([t_ver[-1], t_hor[-1]], dim=1)
